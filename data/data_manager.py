...
class CSVDataManager(object):
    ...
    def __init__(self, config):
        ...
        self.splits = config['splits']
        self.pok_splits = config['pok_splits']

        self.load_func = load_formats[config['format']]
        # ----------------------------------------------------------------------------------------------
        #self.pock_path = config['pock_path']
        pok_mfile = os.path.join('pock.csv')
        pok_metadata_df = pd.read_csv(pok_mfile).sample(frac=1)
        #self.metadata_df = self._remove_too_small(metadata_df, 1)
        self.pok_metadata_df = pok_metadata_df
        self.classes = self._get_classes(self.pok_metadata_df[['class', 'classID']])

        self.pock_data_splits = self.pock_split(self.pok_metadata_df)
    ...
    #remove_too_small은 너무 짦은거 빼기 위해서 근데 우리는 7초 짜리를 넣을거라서 안써도 될듯.
    def _remove_too_small(self, df, min_sec=1):
    ...
    def _10kfold_split(self, df):
        ...
        for s, inds in self.splits.items():
            ...
            df_split = df[df['fold'].isin(inds)] # 컬럼의 값이 inds를 포함하는 것만 골라낸다.
            ret[s] = []
        ...
        return ret
    '''
    def pock_split(self, df):
        ret = {}
        for s, inds in self.pok_splits.items():

            df_split = df[df['fold'].isin(inds)] # 컬럼의 값이 inds를 포함하는 것만 골라낸다.
            ret[s] = []

            for row in df_split[['name', 'class', 'classID']].values:
                fold_mod = 'audio/fold{}'.format(inds[0])
                fname = os.path.join(self.dir_path, fold_mod, '%s'%row[0])
                ret[s].append( {'path':fname, 'class':row[1], 'class_idx':row[2]} )
        return ret

    def get_loader(self, name, transfs):
        assert name in self.pock_data_splits
        dataset = FolderDataset(self.pock_data_splits[name], load_func=self.load_func, transforms=transfs)
